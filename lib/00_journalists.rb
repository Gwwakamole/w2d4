...
def array_counter(twitter)
	puts "Il y a #{twitter.size} journalistes dans l'array."
	puts
end

# Quel est le handle le plus court de cette liste ?
def shortest_finder(twitter)
 	puts "Le handle le plus court est #{twitter.sort_by(&:length)[0]}."
 	puts
end

# Combien y-a-t'il de handle contenant 5 caractères (le @ ne compte pas pour un caractère)
def fiver_counter(twitter)
	fivers = []
	twitter.each do |i|
		if i.length == 6
	    	fivers << i
	...
	puts "Il y a #{fivers.size} handles de 5 caractères."
 	puts
end

# Combien commencent par une majuscule (première lettre juste après le @) ?
def count_start_cap (twitter)
	puts "#{twitter.count {|i| i =~ /^@[[:upper:]]/}} handles commencent par une majuscule"
 	puts
end

# Trie la liste de handle par ordre alphabétique.
def get_alphabetical_order(twitter)
    puts "Voici la liste triée par ordre alphabétique: #{twitter.sort_by(&:downcase)}"
 	puts
end

# Trie la liste de handle par taille des handle (les plus petits en premiers, les plus grands après)
def order_by_length(twitter)
  	puts "Voici la liste triée par longueur des handles: #{twitter.sort_by(&:length)}"
  	puts
end

# Quelle est la position dans l'array de la personne @epenser ?
def position_finder(twitter)
  	puts "@epenser est en position #{twitter.index("@epenser")} dans l'array."
 	puts
end
...
def distribution_maker(twitter)
    1.upto(twitter.sort_by(&:length)[-1].length) do |i|
    	counter = 0
    	twitter.each do |j|
    		if j.length == i
        		counter += 1
...
def perform
	twitter = array
	array_counter(twitter)
	shortest_finder(twitter)
	fiver_counter(twitter)
	count_start_cap (twitter)
	get_alphabetical_order(twitter)
	position_finder(twitter)
	distribution_maker(twitter)
end
